...
export default Region = React.createClass({
  getDefaultProps() {
    return {
      //默认不显示
      visible: false,
  ...
  componentDidMount() {
      fetch('http://pic.ofcard.com/themes/common/region/China_Region_Last.js')
          .then((res) => res.text())
          .then((res) => {eval(res); return CHINA_REGION;})
          .then((data) => {
            //cache it.
            this._data = data;
            ...
            //过滤第一个市对应的区
            var area = [];
            if (this.props.showArea && this.state.selectedCity) {
  ...
  _handleCityChange(city) {
    ...
    //过滤出市变化后，区
    var area = this.props.showArea ? this._filter(city) : [];
    if (area.length === 0) {
      this._selectAreaName = '';
      this.setState({
        selectedCity: city,
